...
describe('LogViewerModal', () => {
  ...
  it('lists log files newest first and auto-selects the newest', async () => {
    getLogFiles.mockResolvedValue([
      file('enclave-2026-08-04.log', 4),
      file('enclave-2026-08-06.log', 6),
      file('enclave-2026-08-05.log', 5),
    ]);
    readLogFile.mockResolvedValue({ success: true, content: 'hello', size: 5, truncated: false });

    render(<LogViewerModal open onOpenChange={() => {}} />);

    await waitFor(() => expect(readLogFile).toHaveBeenCalledWith('/logs/enclave-2026-08-06.log'));
    const names = Array.from(
      screen.getByTestId('log-file-list').querySelectorAll('button')
    ).map((b) => b.textContent);
    expect(names[0]).toContain('enclave-2026-08-06.log');
    expect(names[2]).toContain('enclave-2026-08-04.log');
  });

  it('shows a truncation notice, the full size, and the elided amount when truncated', async () => {
    getLogFiles.mockResolvedValue([file('enclave-2026-08-06.log', 6, 3 * 1024 * 1024)]);
  ...
  it('does not claim truncation for a complete file', async () => {
    getLogFiles.mockResolvedValue([file('enclave-2026-08-06.log', 6, 512)]);
  ...
  it('surfaces a failed read envelope instead of blank content', async () => {
    getLogFiles.mockResolvedValue([file('enclave-2026-08-06.log', 6)]);
  ...
  it('switches files on selection', async () => {
    ...
    fireEvent.click(screen.getByText('enclave-2026-08-05.log'));
    await waitFor(() => expect(readLogFile).toHaveBeenCalledWith('/logs/enclave-2026-08-05.log'));
  });
