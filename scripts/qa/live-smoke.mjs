 ...
 *
 * Skips cleanly (exit 0) when no ENCLAVE_QA_*_KEY is set, so it's safe to wire
...
if (!QA_KEYS.some(k => process.env[k])) {
  console.log('SKIP: no ENCLAVE_QA_*_KEY in .env.local — live smoke skipped (expected until a key is added).');
  process.exit(0);
}

const provider = process.env.ENCLAVE_QA_PROVIDER || 'openrouter';
const model = process.env.ENCLAVE_QA_MODEL || 'z-ai/glm-4.6';
